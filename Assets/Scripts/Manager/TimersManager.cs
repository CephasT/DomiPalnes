...
public class TimersManager : IGameSystemMono
{
    ...
    private void FindAndRemove(UnityAction UnityAction)
    {
    ...
    public void SetTimer(object Owner, Timer timer)
    {
    ...
    public void SetTimer(object Owner, float interval, uint LoopsCount, UnityAction unityAction)
    {
    ...
    /// <param name="interval">Interval(in seconds) between loops</param>
    /// <param name="UnityAction">Delegate</param>
    public void SetTimer(object Owner, float interval, UnityAction unityAction)
    {
    ...
    /// <param name="unityAction">Delegate</param>
    public void SetLoopableTimer(object Owner, float interval, UnityAction unityAction)
    {
        if (unityAction != null && interval > 0f && Owner != null)
        {
            ClearTimer(unityAction);
    ...
    public void AddTimers(object Owner, List<Timer> Timers)
    {
        if (Owner == null)
        {
#if UNITY_EDITOR
    ...
    public void ClearTimer(UnityAction UnityAction)
    {
        if (UnityAction != null)
    ...
    /// </summary>
    /// <param name="UnityAction">Delegate name</param>
    public Timer GetTimerByName(UnityAction UnityAction)
    {
        foreach (KeyValuePair<WeakReference, Timer> elem in m_Timers)
            if (elem.Value.Delegate() == UnityAction)
                return elem.Value;
    ...
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public float Interval(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? 0f : timer.Interval(); }

    /// <summary>
    /// Get total loops count (INFINITE (which is uint.MaxValue) if is constantly looping) 
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public uint LoopsCount(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? 0 : timer.LoopsCount(); }

    /// <summary>
    /// Get how many loops were completed
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public uint CurrentLoopsCount(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? 0 : timer.CurrentLoopsCount(); }

    /// <summary>
    /// Get how many loops remained to completion
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public uint RemainingLoopsCount(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? 0 : timer.RemainingLoopsCount(); }

    /// <summary>
    /// Get total remaining time
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public float RemainingTime(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? -1f : timer.RemainingTime(); }

    /// <summary>
    /// Get total elapsed time
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public float ElapsedTime(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? -1f : timer.ElapsedTime(); }

    /// <summary>
    /// Get elapsed time in current loop
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public float CurrentCycleElapsedTime(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? -1f : timer.CurrentCycleElapsedTime(); }

    /// <summary>
    /// Get remaining time in current loop
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public float CurrentCycleRemainingTime(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? -1f : timer.CurrentCycleRemainingTime(); }

    /// <summary>
    /// Verifies whether the timer exits
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public bool IsTimerActive(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer != null; }

    /// <summary>
    /// Checks if the timer is paused
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public bool IsTimerPaused(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? false : timer.IsPaused(); }
    ...
    public void SetPaused(UnityAction unityAction, bool bPause) { Timer timer = GetTimerByName(unityAction); if (timer != null) timer.SetPaused(bPause); }

    /// <summary>
    /// Get total duration, (INFINITE if it's constantly looping)
    /// </summary>
    /// <param name="unityAction">Delegate name</param>
    public float Duration(UnityAction unityAction) { Timer timer = GetTimerByName(unityAction); return timer == null ? 0f : timer.Duration(); }
